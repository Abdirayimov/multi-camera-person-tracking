...
find_library(DeepStream_NVBUFSURFACE_LIB NAMES nvbufsurface
    HINTS ${_DS_ROOTS} PATH_SUFFIXES lib)
find_library(DeepStream_NVDSGST_META_LIB NAMES nvdsgst_meta
    HINTS ${_DS_ROOTS} PATH_SUFFIXES lib)

if(DeepStream_INCLUDE_DIR)
    # The install may be reached through the unversioned /deepstream symlink,
    # so read the version from the header rather than from the path.
    if(EXISTS "${DeepStream_INCLUDE_DIR}/nvds_version.h")
        file(STRINGS "${DeepStream_INCLUDE_DIR}/nvds_version.h" _ds_major
             REGEX "#define NVDS_VERSION_MAJOR")
        file(STRINGS "${DeepStream_INCLUDE_DIR}/nvds_version.h" _ds_minor
             REGEX "#define NVDS_VERSION_MINOR")
        string(REGEX MATCH "[0-9]+" _ds_major "${_ds_major}")
        string(REGEX MATCH "[0-9]+" _ds_minor "${_ds_minor}")
        set(DeepStream_VERSION "${_ds_major}.${_ds_minor}")
    else()
...
if(DeepStream_FOUND)
    ...
    if(NOT TARGET DeepStream::nvdsgst_meta)
        add_library(DeepStream::nvdsgst_meta UNKNOWN IMPORTED)
        set_target_properties(DeepStream::nvdsgst_meta PROPERTIES
            IMPORTED_LOCATION "${DeepStream_NVDSGST_META_LIB}"
            INTERFACE_INCLUDE_DIRECTORIES "${DeepStream_INCLUDE_DIR}")
    endif()
